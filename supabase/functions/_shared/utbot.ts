// The UT Bot signal, as pure arithmetic over bars — the one copy of it.
//
// This is `src/lib/signals.ts` with everything browser-shaped taken out. It was
// extracted rather than transcribed, and that distinction is the whole reason
// the file is here: `notify-signals` pushes an alert saying a symbol flipped,
// the table in front of the user says whether it did, and two implementations
// of six hundred lines of trailing-stop arithmetic would sooner or later
// disagree about a symbol somebody had money on.
//
// `_shared/yahoo.ts` deliberately *transcribes* `technicals.ts` and says so.
// That was right for thirty lines of RSI and is wrong here.
//
// Two rules keep one file working in both runtimes:
//
//   1. **No imports.** Deno resolves nothing, so `Candle` is declared below
//      rather than imported from `../../../src/types` — it is structurally
//      identical, and TypeScript is structural, so values pass both ways.
//   2. **No globals.** `latestSignal` used to call `isMarketOpen()` from
//      `src/lib/format.ts` to decide `provisional`; it now takes that answer as
//      an argument. The browser passes the real clock, the Edge Function passes
//      its own, and a test passes neither and gets a deterministic result.
//
// The browser reaches *out* to this file (`src/lib/signals.ts` re-exports it),
// not the other way round. That direction is deliberate: the Supabase CLI
// bundler is the fussy one about imports outside its own tree, Vite is not.

/**
 * One daily bar. Structurally identical to `Candle` in `src/types.ts` — declared
 * here instead of imported so this file has no dependencies at all.
 */
...
export function latestSignal(
  ...
  cfg = UT_BOT,
  /**
   * Is the session open right now? Decides `provisional` alone.
   *
   * An argument rather than a call, because this file has no browser in it. The
   * default is the safe answer for a caller that does not know: a flip treated
   * as settled is only ever wrong about the score, whereas defaulting to `true`
   * would dock ten points off every signal computed off-hours.
   */
  isOpen = false,
): Signal | null {
  ...
  // fact. The caller knows the session; see `isOpen` above.
  const provisional = flip.index === last && isOpen;
