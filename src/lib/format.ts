 ...
 *
 * `mapPool` bounds its own pool, which is enough when one pool is all there is.
 * Callers with no list to map over — the per-row signal fetches in signals.ts,
 * each started independently by a cell scrolling into view — have no such
 * shape, and one gate is what keeps their combined ceiling a number somebody
 * measured rather than "however many rows are on screen".
 */
